import streamlit as st
from supabase import create_client, Client

# --- 1. إعدادات الهوية ---
VERSION = "0.8"
DEV_NAME = "Mohammad-Sofian"
DEV_LOG_PWD = "Soffian3491335"

st.set_page_config(page_title="سكنات شكّور", layout="wide")

# --- 2. الربط بالسيرفر ---
try:
    URL = st.secrets["SUPABASE_URL"]
    KEY = st.secrets["SUPABASE_KEY"]
    ADMIN_PWD = st.secrets["ADMIN_PASSWORD"]
    supabase: Client = create_client(URL, KEY)
except:
    st.error("⚠️ تأكد من ضبط Secrets (URL, KEY, ADMIN_PASSWORD)")
    st.stop()

# --- 3. تصميم CSS بسيط (لا يخرب الشاشة ولا الإيقونات) ---
st.markdown("""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Cairo&display=swap');
    /* تطبيق الخط العربي فقط على النصوص لمنع تشوه الإيقونات */
    html, body, [class*="st-"], p, h1, h2, h3 {
        font-family: 'Cairo', sans-serif !important;
        direction: rtl;
        text-align: right;
    }
    .st-card {
        background: white; padding: 15px; border-radius: 10px;
        box-shadow: 0 2px 5px rgba(0,0,0,0.1); border-right: 6px solid #2E86C1;
        margin-bottom: 12px;
    }
    </style>
    """, unsafe_allow_html=True)

# --- 4. نظام الدخول ---
if "logged_in" not in st.session_state:
    st.session_state["logged_in"] = False

if not st.session_state["logged_in"]:
    st.markdown("<h2 style='text-align: center;'>🏢 دخول سكنات شكّور</h2>", unsafe_allow_html=True)
    c1, c2, c3 = st.columns([1, 2, 1])
    with c2:
        pwd_in = st.text_input("🔑 كلمة المرور الإدارية", type="password")
        if st.button("دخول للنظام", use_container_width=True) or (pwd_in == ADMIN_PWD and pwd_in != ""):
            if pwd_in == ADMIN_PWD:
                st.session_state["logged_in"] = True
                # تسجيل دخول جهاز (بسيط)
                try: supabase.table("login_logs").insert({"device_info": "دخول جديد"}).execute()
                except: pass
                st.rerun()
            elif pwd_in != "": st.error("❌ كلمة المرور خطأ")
    st.stop()

# --- 5. جلب البيانات ---
@st.cache_data(ttl=2)
def load_data():
    s_res = supabase.table("sakanat").select("*").order('name').execute()
    t_res = supabase.table("students").select("*, sakanat(name)").order('created_at', desc=True).execute()
    return s_res.data, t_res.data

s_list, t_list = load_data()

# --- 6. القائمة الجانبية (Sidebar) ---
with st.sidebar:
    st.header("⚙️ الإدارة")
    st.write(f"المطور: **{DEV_NAME}**")
    search_q = st.text_input("🔍 بحث بالاسم:")
    st.markdown("---")
    
    # ركن المطور المصلح (يعمل فوراً بالرمز)
    st.write("🛠️ ركن المطور")
    dev_key_in = st.text_input("رمز المطور", type="password", key="dev_key_unique")
    if dev_key_in == DEV_LOG_PWD:
        try:
            logs = supabase.table("login_logs").select("*").order('login_time', desc=True).limit(5).execute()
            for l in logs.data:
                st.caption(f"🕒 {l['login_time'][11:16]} | {l['device_info']}")
        except: st.error("الجدول login_logs غير موجود في سوبابيس")
    
    if st.button("🚪 تسجيل الخروج"):
        st.session_state["logged_in"] = False
        st.rerun()

# --- 7. الواجهة الرئيسية (Tabs) ---
tab1, tab2 = st.tabs(["👥 قائمة الطالبات", "📊 إحصائيات"])

with tab1:
    s_names = ["الكل"] + [s['name'] for s in s_list]
    choice = st.selectbox("📍 تصفية حسب الشقة:", s_names)
    
    filtered = t_list
    if choice != "الكل":
        filtered = [s for s in t_list if s.get('sakanat') and s['sakanat']['name'] == choice]
    if search_q:
        filtered = [s for s in filtered if search_q.lower() in s['name'].lower()]

    for s in filtered:
        sid = str(s['id'])
        wa_link = f"https://wa.me/962{str(s['phone'])[1:]}" if str(s['phone']).startswith('07') else "#"

        st.markdown(f"""
            <div class="st-card">
                <h3 style="color:#2E86C1; margin:0;">👤 {s['name']}</h3>
                <p style="margin:5px 0;">🏠 شقة: {s.get('sakanat', {}).get('name', 'N/A')} | 📞 {s['phone']}</p>
                <p style="color:gray; font-size:14px;">📝 {s['notes'] if s['notes'] else 'لا ملاحظات'}</p>
            </div>
        """, unsafe_allow_html=True)
        
        c1, c2, c3 = st.columns([1, 2, 1])
        with c1: st.link_button("💬 واتساب", wa_link, use_container_width=True)
        
        with c2:
            fc = st.columns(3)
            files = [("هوية", 'file_id'), ("عقد", 'file_contract'), ("كمبيالة", 'file_kumbiala')]
            for i, (lab, col) in enumerate(files):
                # حل مشكلة الـ Duplicate ID بإضافة key فريد
                if s.get(col):
                    url = supabase.storage.from_("student_files").get_public_url(s[col])
                    fc[i].link_button(lab, f"{url}?download=", use_container_width=True, key=f"btn_{col}_{sid}")
                else:
                    fc[i].button(f"❌ {lab}", disabled=True, use_container_width=True, key=f"none_{col}_{sid}")
        
        with c3:
            with st.popover("⚙️ خيارات", use_container_width=True):
                n_v = st.text_input("الاسم", s['name'], key=f"un_{sid}")
                p_v = st.text_input("الهاتف", s['phone'], key=f"up_{sid}")
                m_v = st.text_area("الملاحظات", s['notes'], key=f"um_{sid}")
                if st.button("حفظ ✅", key=f"save_{sid}"):
                    supabase.table("students").update({"name": n_v, "phone": p_v, "notes": m_v}).eq("id", sid).execute()
                    st.rerun()
                st.markdown("---")
                if st.checkbox("تأكيد الحذف؟", key=f"conf_{sid}"):
                    if st.button("حذف نهائي", key=f"del_{sid}"):
                        supabase.table("students").delete().eq("id", sid).execute()
                        st.rerun()

with tab2:
    st.metric("إجمالي الطالبات", len(t_list))
    st.metric("عدد الشقق", len(s_list))

st.caption(f"v{VERSION} | {DEV_NAME}")